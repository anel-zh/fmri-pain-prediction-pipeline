function voxel_beta_dat_all = HH_mint_a1_pain_as_regressor_spec(config_structure, participants_option, varargin)
...
for participant_i = 1:numel(participants_to_analyze)
    ...
    if ismember('caps', run_to_analyze)
        ...
        nuisance_caps.R = nuisance_caps.R((rm_TR_start+1):(end-rm_TR_end-1),:);
    end
    if ismember('quinine', run_to_analyze)
        ...
        nuisance_quinine.R = nuisance_quinine.R((rm_TR_start+1):(end-rm_TR_end-1),:);
    end
    ...
    for run_i = 1:numel(run_files_all)
        ...
        behav_all = config_structure.behavioral_data.specificity.(run_name{1}); %main can be changed to spec in the future for convinience.

        % Loading fmri data
        run_path = fullfile(participant_dir, run_file.name);

        % Matching the duration (all to the same original)
       
        fmri_img_orig = fmri_data(run_path, which('gray_matter_mask.nii'));
        
        fmri_img = fmri_img_orig;
        fmri_img.dat = fmri_img.dat(:,(rm_TR_start+1):(end-rm_TR_end-1));
        fmri_img.fullpath = fmri_img.fullpath((rm_TR_start+1):(end-rm_TR_end-1),:);
        fmri_img.files_exist = fmri_img.files_exist((rm_TR_start+1):(end-rm_TR_end-1),:);
