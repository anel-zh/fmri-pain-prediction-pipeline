function [d, Z, transform] = HH_procrustes_time_mint(X, Y, varargin)
...
% Center across time (rows).
muX = mean(X, 2); % Mean across rows (time) for each column (feature)
muY = mean(Y, 2); % Mean across rows (time) for each column (feature)
X0 = X - repmat(muX, 1, size(X, 2)); % Subtract mean for each time point
Y0 = Y - repmat(muY, 1, size(Y, 2)); % Subtract mean for each time point

% Compute sum of squares across time (rows).
ssqX = sum(X0.^2, 2); % Sum squared deviations across columns
ssqY = sum(Y0.^2, 2); % Sum squared deviations across columns
constX = all(ssqX <= abs(eps(class(X)) * size(X, 2) * muX).^2, 1); % Check for near-constant rows
constY = all(ssqY <= abs(eps(class(X)) * size(X, 2) * muY).^2, 1); % Check for near-constant rows
ssqX = sum(ssqX); % Total sum of squares for X
ssqY = sum(ssqY); % Total sum of squares for Y
