function [pain_score, binned] = HH_specificity_binning_applying_model_weights_v2_binned_C(model_data, behavioral_test, connectivity_test, activation_test, brain_measure, run, varargin)
% Duration
window_size = 90;
...
switch brain_measure
    case 'connectivity_and_hrf_voxel'
        % ========= Brain Data
        binned.connectivity_caps = connectivity_test.caps;
        if isfield(connectivity_test, 'quinine')
            binned.connectivity_quinine = connectivity_test.quinine;
        end
        binned.connectivity_rest = connectivity_test.rest;
    ...
    case 'connectivity'
        binned.caps = connectivity_test.caps;
        if isfield(connectivity_test, 'quinine')
            binned.quinine = connectivity_test.quinine;
        end
        binned.rest = connectivity_test.rest;
        wei_name = 'C_wei';
